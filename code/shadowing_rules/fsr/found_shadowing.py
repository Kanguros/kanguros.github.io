...
ValueAny = list("any")
"""
Helper variable to represent ``list['any']`` in order 
...
def check_source_zone(rule: SecurityRule, preceding_rule: SecurityRule) -> Result:
    """Checks if the source zones are identical or if the preceding rule allows any zone."""
    result = rule.source_zones == preceding_rule.source_zones or ValueAny == preceding_rule.source_zones
    message = "Preceding rule source zones cover rule's source zones" if result else "Source zones differ"
    return result, message
...
def check_destination_zone(
    ...
    result = rule.source_zones == preceding_rule.source_zones or ValueAny == preceding_rule.source_zones
    message = "Preceding rule destination zones cover rule's destination zones" if result else "Destination zones differ"
    return result, message
...
def check_destination_address(
    ...
    for addr in rule.destination_addresses_ip:
        if not any(
                addr.subnet_of(net) for net in preceding_rule.destination_addresses_ip
        ):
            return False, f"Address {addr} is not covered by preceding rule"
...
def check_application(rule: SecurityRule, preceding_rule: SecurityRule) -> Result:
...
def check_services(rule: SecurityRule, preceding_rule: SecurityRule) -> Result:
    ...
    if all(rule_service in preceding_rule.services for rule_service in rule.services):
        return True, "Preceding rule contains rule's applications"
    return False, "Preceding rule does not contain all rule's applications"
